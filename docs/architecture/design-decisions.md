...
**判断**: jaconvを使用して全角/半角両方で検索し、**入力そのままも候補に残す**

**理由**:
- 日本語検索では全角/半角の違いが問題になる
- ユーザーは入力形式を意識したくない
- 検索精度が向上する
- 変換は語全体へ一律に掛かるので、1語の中で幅が混ざる値 (`ABCビル`) は
  どちらの変換結果とも一致しない。生の入力を落とすと「格納値をそのまま打てば
  引ける」という素の `icontains` の保証を失う

**トレードオフ**:
- 1語あたりのLIKEが最大3本になる (候補はsetなので、単一表記の値では2本のまま)
- パフォーマンスへの影響
